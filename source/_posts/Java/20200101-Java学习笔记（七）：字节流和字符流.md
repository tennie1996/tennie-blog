...
<!-- 
>[foochane](https://foochane.cn/) ：[https://foochane.cn/article/2020010101.html](https://foochane.cn/article/2020010101.html) 
...
![](https://raw.githubusercontent.com/foochane/java-learning/master/image/1_io.jpg)
...
`java.io.FileOutputStream `类是文件输出流，用于把内存中的数据写入到硬盘的文件中。
...
经过以上的演示，每次程序运行，创建输出流对象，都会**清空**目标文件中的数据。如何保留目标文件中数据，还能继续添加新数据呢？
...
![](https://raw.githubusercontent.com/foochane/java-learning/master/image/2_copy.jpg)
...
public class FRRead {
    public static void main(String[] args) throws IOException {
      	// 使用文件名称创建流对象
       	FileReader fr = new FileReader("test.txt"); //新年新气象
      	// 定义变量，保存数据
        int b;
        // 循环读取
        while ((b = fr.read())!=-1) {
